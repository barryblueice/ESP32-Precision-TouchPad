...
## **关于模型:**
 ...
 - `触摸板.SLDPRT`是我所使用的触摸板的范例模型。
 - `指纹模块.SLDPRT`是HP FM3483/3633指纹模块的范例模型。
 - `3D_PCB1_2026-01-18.step`是触摸板主PCB的范例模型。
 - `外壳.SLDPRT` / `外壳.STEP`是触摸板外壳，`底壳.SLDPRT` / `底壳.STEP` 是外壳底座。
 - `指纹模块固定板.step`是用于固定指纹模块到外壳上的PCB。
 - `总装.SLDASM`是整个产品的总装+理论测试+渲染图。
 - `外壳-无指纹.SLDPRT` / `外壳-无指纹.STEP`是不带指纹模块的触摸板外壳，`底壳-无指纹.SLDPRT` / `底壳-无指纹.STEP` 是不带指纹模块的外壳底座。
  - `总装-无指纹.SLDASM`是不带指纹模块的整个产品的总装+理论测试+渲染图。
